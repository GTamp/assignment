function countdir {	#function that counts the # of directories
        ...
        dircount=$(ls -l | egrep "^-" | wc -l)	#count the # of results that ls -l returns and start with d (i.e directory instead of file which starts with -)
...
do
	if [[ $guess -eq $fnum ]]	#if players guess matches the functions calculation
	then
		echo "You guessed right! Congratulations!"
		echo "The game has ended."
		loopstop=1	#raise the flag to break the loop
	elif [[ $guess -gt $fnum ]]	#if players guess is to high
	then
